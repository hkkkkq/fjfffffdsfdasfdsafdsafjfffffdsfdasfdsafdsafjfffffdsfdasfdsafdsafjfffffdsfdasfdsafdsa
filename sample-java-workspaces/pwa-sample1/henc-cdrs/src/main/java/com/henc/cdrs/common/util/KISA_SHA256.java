...
public class KISA_SHA256 {
    ...
    public static String encryptPasswordPortalStyle(String password) {
        byte[] pbData = new byte[password.length()];
        int i;
        for (i = 0; i < password.length(); i++) {
            pbData[i] = (byte) (password.charAt(i) & 0xff);
        }
        ...
        for (i = 0; i < pbCipher.length; i++) {
            String x = Integer.toHexString(pbCipher[i] & 0xff).toUpperCase();
            if (x.length() < 2) {
                r.append("0");
            }
            r.append(x);
    ...
    public static void main(String[] args) {
        String inpara = "star6349!!";
        StringBuffer r = new StringBuffer();

        byte[] pbData = new byte[inpara.length()];

        for (int i = 0; i < inpara.length(); i++) {
            pbData[i] = (byte) (inpara.charAt(i) & 0xff);
        }

        byte pbCipher[] = new byte[32];

        SHA256_Encrpyt(pbData, pbData.length, pbCipher);

        for (int i = 0; i < pbCipher.length; i++) {
            String x = Integer.toHexString(0xff & pbCipher[i]).toUpperCase();
            if (x.length() < 2) {
                r.append("0");
            }
            r.append(x);
        }
        System.out.println(r.toString() + " : " + r.toString().length());
    }
}
