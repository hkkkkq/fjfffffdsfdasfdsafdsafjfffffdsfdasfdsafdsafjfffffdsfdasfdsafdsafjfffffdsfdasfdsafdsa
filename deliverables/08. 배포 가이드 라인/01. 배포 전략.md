...
| | 설명 |
|:---:|---|
| 설명 | 기본 CI 파이프라인으로 image build이후 pod의 이미지를 빌드한 이미지로 교체한다. 초기 환경 구성이후에는 kubernetes manifest부분의 변경이 특별한 사유 빼고는 필요없기 때문에 이 잡을 기본으로 사용한다.|
|위치 | 개발 Jenkins |
...
> 잡을 선택후 "Build with Parameters"를 선택한다.
...
|Stage|설명|
|----|-----|
|Deploy to Cluster|M대상 Pod의 이미지를 변경 한다.|
``` groovy
stage( 'Deploy to Cluster' ) {
		container('oc') {
              /* rolling update */
