...
//2. CustDtlInfoRetvSO  스크립트 작업 테스트
Action21()
{

//	int EncYN = 1;			//암호화 여부    0=평문     1:암호화

	char* data_RequestXml_21;				//요청 평문 XML
	char data_RequestXmlEnc_21[20000];		//요청 Encoded XML


	char* data_ResponseXmlENC_21;			//응답 암호화문 XML    
	char* data_ResponseXml_21; 				//응답 Deoded XML
...
//    char cipherText[4000];
 ...
 if (EncYN == 0) {		//평문 조회
	...
	//lr_start_transaction("BD_고객요약정보 조화_CustDtlInfoRetvSO");

	 //20160517_wonho
	 //lr_resume_transaction("02_검색서비스번호 요약조회_ASIS");
   ...
   //lr_end_transaction("BD_고객요약정보 조화_CustDtlInfoRetvSO", LR_AUTO);
	  //lr_stop_transaction("02_검색서비스번호 요약조회_TOBE");
	 lr_end_transaction("02_검색서비스번호 요약조회_TOBE",LR_AUTO);
	...
	//lr_resume_transaction("02_검색서비스번호 요약조회_checkSum");
...
//	web_reg_save_param("Data_encrypt_res",  "LB=\r\n\r\n",  "RB=",  LAST);
	...
	//lr_start_transaction("BE_고객요약정보 조화_CustDtlInfoRetvSO");
...
/*
	  ...
	  // Endcoe/Decode 비교 ransaction 위치 (03.25)
	  */
	  //lr_stop_transaction("02_검색서비스번호 요약조회_checkSum");
	   lr_end_transaction("02_검색서비스번호 요약조회_checkSum",LR_AUTO);
