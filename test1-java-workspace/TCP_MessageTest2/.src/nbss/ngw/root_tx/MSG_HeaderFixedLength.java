...
/**
 * @file              nbss.ngw.root_tx.MSG_HeaderFixedLength.java
...
public class MSG_HeaderFixedLength extends com.tmax.promapper.engine.base.FixedLengthMessage implements ResourceMeta {
	...
	public MSG_HeaderFixedLength() {
		super();
	}

	public MSG_HeaderFixedLength(int conversionType) {
		super(conversionType);
	}

	public MSG_HeaderFixedLength(String charsetName) {
		super(charsetName);
	}
	/* marshall method */
	public byte [] marshalObject(Object obj) throws Exception {
		FixedLengthMarshalFieldV2 flFld = new FixedLengthMarshalFieldV2();
		nbss.ngw.root_tx.MSG_Header __root = (nbss.ngw.root_tx.MSG_Header)obj;
		...
		if(__root == null) {
			...
			if(logger.isLoggable(java.util.logging.Level.FINEST)) { 
			 	logger.finest("FieldName = data1,  StartOffset = " + flFld.getTotalLength() + " Length=4, fill= , decimal=0, point=false, sign=false, encodeOption=Constants.ENCODE_CHAR, EOD = false");
			}
			flFld.setStringField(this, __root.getData1(), 4, Constants.ALIGN_NON, " ", 0, false, false, Constants.ENCODE_CHAR, null);
		}
	...
	public Object unmarshalObject(byte[] message, int startOffset) throws Exception{
		nbss.ngw.root_tx.MSG_Header dataObject = new nbss.ngw.root_tx.MSG_Header();
	...
	public int unmarshalObject(byte[] message, int startOffset, Object dataObject) throws Exception {		
		if(logger.isLoggable(Level.FINEST)) {
			logger.finest("#Unmarshall Start. Message Class = [MSG_HeaderFixedLength], byte[] length = " + message.length + ", Start Offset = " + startOffset);
		}

		nbss.ngw.root_tx.MSG_Header __root = (nbss.ngw.root_tx.MSG_Header)dataObject;
		FixedLengthUnmarshalField flFld = new FixedLengthUnmarshalField(message, startOffset);
		

		/** data1 */
		if(logger.isLoggable(java.util.logging.Level.FINEST)) {
			logger.finest("FieldName = data1,  StartOffset = " + flFld.getOffset() + " Length=4, fill= , decimal=0, point=false, sign=false, encodeOption=Constants.ENCODE_CHAR, trimFlag=default, rootTrimFlag=rtrim");
		}
		__root.setData1(flFld.getStringField(this, 4, Constants.ALIGN_NON, " ", 0, false, false, Constants.ENCODE_CHAR, Constants.TRIM_RIGHT));
	...
	MessageFieldProperty fldProp = null;
		fldProp = new MessageFieldProperty("data1", "data1", MessageFieldType.STRING, (String)null, null, "4", Constants.ALIGN_NON, " ", 0, false, false, null, null, null, null, null, Constants.TRIM_RIGHT, Constants.ENCODE_CHAR);
	...
	public String getMetaData(String input) {
		...
		if (input.equalsIgnoreCase(META_PHYSICAL_NAME)) {
			...
			return "nbss.ngw.root_tx:MSG_HeaderFixedLength.msg";
