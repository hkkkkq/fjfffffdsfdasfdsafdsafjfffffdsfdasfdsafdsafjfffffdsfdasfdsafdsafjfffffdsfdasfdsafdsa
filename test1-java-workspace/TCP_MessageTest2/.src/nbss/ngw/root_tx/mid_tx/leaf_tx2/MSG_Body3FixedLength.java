...
/**
 * @file              nbss.ngw.root_tx.mid_tx.leaf_tx2.MSG_Body3FixedLength.java
...
public class MSG_Body3FixedLength extends com.tmax.promapper.engine.base.FixedLengthMessage implements ResourceMeta {
	...
	public MSG_Body3FixedLength() {
		super();
	}

	public MSG_Body3FixedLength(int conversionType) {
		super(conversionType);
	}

	public MSG_Body3FixedLength(String charsetName) {
		super(charsetName);
	}
	/* marshall method */
	public byte [] marshalObject(Object obj) throws Exception {
		FixedLengthMarshalFieldV2 flFld = new FixedLengthMarshalFieldV2();
		nbss.ngw.root_tx.mid_tx.leaf_tx2.MSG_Body3 __root = (nbss.ngw.root_tx.mid_tx.leaf_tx2.MSG_Body3)obj;
		...
		if(__root == null) {
			...
			if(logger.isLoggable(java.util.logging.Level.FINEST)) { 
			 	logger.finest("FieldName = data5,  StartOffset = " + flFld.getTotalLength() + " Length=10, fill= , decimal=0, point=false, sign=false, encodeOption=Constants.ENCODE_CHAR, EOD = false");
			}
			flFld.setStringField(this, __root.getData5(), 10, Constants.ALIGN_NON, " ", 0, false, false, Constants.ENCODE_CHAR, null);
		}
	...
	public Object unmarshalObject(byte[] message, int startOffset) throws Exception{
		nbss.ngw.root_tx.mid_tx.leaf_tx2.MSG_Body3 dataObject = new nbss.ngw.root_tx.mid_tx.leaf_tx2.MSG_Body3();
	...
	public int unmarshalObject(byte[] message, int startOffset, Object dataObject) throws Exception {		
		if(logger.isLoggable(Level.FINEST)) {
			logger.finest("#Unmarshall Start. Message Class = [MSG_Body3FixedLength], byte[] length = " + message.length + ", Start Offset = " + startOffset);
		}

		nbss.ngw.root_tx.mid_tx.leaf_tx2.MSG_Body3 __root = (nbss.ngw.root_tx.mid_tx.leaf_tx2.MSG_Body3)dataObject;
		FixedLengthUnmarshalField flFld = new FixedLengthUnmarshalField(message, startOffset);
		

		/** data5 */
		if(logger.isLoggable(java.util.logging.Level.FINEST)) {
			logger.finest("FieldName = data5,  StartOffset = " + flFld.getOffset() + " Length=10, fill= , decimal=0, point=false, sign=false, encodeOption=Constants.ENCODE_CHAR, trimFlag=default, rootTrimFlag=rtrim");
		}
		__root.setData5(flFld.getStringField(this, 10, Constants.ALIGN_NON, " ", 0, false, false, Constants.ENCODE_CHAR, Constants.TRIM_RIGHT));
	...
	MessageFieldProperty fldProp = null;
		fldProp = new MessageFieldProperty("data5", "data5", MessageFieldType.STRING, (String)null, null, "10", Constants.ALIGN_NON, " ", 0, false, false, null, null, null, null, null, Constants.TRIM_RIGHT, Constants.ENCODE_CHAR);
		fieldPropertyMap.put("data5", fldProp);
	...
	public String getMetaData(String input) {
		...
		if (input.equalsIgnoreCase(META_PHYSICAL_NAME)) {
			...
			return "nbss.ngw.root_tx.mid_tx.leaf_tx2:MSG_Body3FixedLength.msg";
